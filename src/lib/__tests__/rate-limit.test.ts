...
describe('checkRateLimit', () => {
  ...
  it('returns null when under limit', async () => {
    const limiter = { limit: vi.fn().mockResolvedValue({ success: true, limit: 5, remaining: 4, reset: Date.now() + 10000 }) } as unknown as Ratelimit;
    const result = await checkRateLimit(req, limiter);
    expect(result).toBeNull();
  });

  it('returns 429 when over limit', async () => {
    const limiter = { limit: vi.fn().mockResolvedValue({ success: false, limit: 5, remaining: 0, reset: Date.now() + 10000 }) } as unknown as Ratelimit;
    const result = await checkRateLimit(req, limiter);
  ...
  it('includes rate limit headers on 429', async () => {
    ...
    const limiter = { limit: vi.fn().mockResolvedValue({ success: false, limit: 5, remaining: 0, reset }) } as unknown as Ratelimit;
