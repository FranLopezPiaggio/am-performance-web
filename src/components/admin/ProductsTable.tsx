...
import { Package, ExternalLink, ImageUp, ChevronDown, ChevronRight, Save, ChevronLeft, ChevronRight as ChevronRightIcon } from 'lucide-react';
...
export default function ProductsTable() {
  ...
  const { products, loading, error, total } = useProducts(
    { isActive: null as unknown as boolean | undefined, _t: refreshKey } as never,
    200
  );
  ...
  // Si la página actual ya no existe (ej: se eliminó el último producto de la página)
  useEffect(() => {
      ...
      <div className="overflow-x-auto">
            ...
            {paginatedProducts.map((p) => {
              ...
              return (
                <React.Fragment key={p.id}>
                  <tr className={`border-b border-white/5 hover:bg-white/5 transition-colors ${!p.is_active ? 'opacity-50' : ''}`}>
                    ...
                    <td className="px-4 py-4 text-white/30 uppercase tracking-widest text-[10px]">
                      {p.line?.name ?? <span className="text-white/10">—</span>}
                    </td>
                    ...
                    <td className="px-4 py-4 text-center">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        {!p.is_active && (
                          <span className="bg-red-500/20 text-red-400 text-[10px] px-2 py-1 font-bold uppercase tracking-widest">
                            Inactivo
                          </span>
                        )}
                      </div>
        ...
        <div className="flex items-center justify-between p-4 border-t border-white/10">
          <p className="text-[10px] uppercase tracking-widest text-white/30">
            Mostrando {(currentPage - 1) * pageSize + 1}–{Math.min(currentPage * pageSize, products.length)} de {products.length}
          </p>
          <div className="flex items-center gap-3">
            <button
              onClick={() => setCurrentPage((p) => Math.max(1, p - 1))}
              disabled={currentPage <= 1}
              className="inline-flex items-center gap-1 text-[10px] uppercase tracking-widest text-white/40 hover:text-white transition-colors disabled:opacity-20 disabled:cursor-not-allowed"
            >
              <ChevronLeft size={12} />
              Anterior
            </button>
